...
void CFilamentSimulator::initSimulator()
{
	...
	if (FILE* file = fopen(occupancy3D_data.c_str(), "r"))
	{
		...
		if (verbose) ROS_INFO("[filament] Env dimensions (%.2f,%.2f,%.2f) to (%.2f,%.2f,%.2f)", environment.description.min_coord.x, environment.description.min_coord.y, environment.description.min_coord.z, environment.description.max_coord.x, environment.description.max_coord.y, environment.description.max_coord.z);
		if (verbose) ROS_INFO("[filament] Env size in cells	 (%d,%d,%d) - with cell size %f [m]", environment.description.num_cells.x, environment.description.num_cells.y, environment.description.num_cells.z, environment.description.cell_size);
	...
	double   end_x, double   end_y, double end_z)
{
	...
	for (int i = 1; i < steps - 1; i++)
	{
		...
		if (environment.Env[indexFrom3D(x_idx, y_idx, z_idx)] != 0) { return PATH_OBSTRUCTED; }
	}
...
void CFilamentSimulator::save_state_to_file()
{
	...
	ist.write((char*)&environment.description.min_coord.z, sizeof(double));

	ist.write((char*)&environment.description.max_coord.x, sizeof(double));
	ist.write((char*)&environment.description.max_coord.y, sizeof(double));
	ist.write((char*)&environment.description.max_coord.z, sizeof(double));

	ist.write((char*)&environment.description.num_cells.x, sizeof(int));
	ist.write((char*)&environment.description.num_cells.y, sizeof(int));
	ist.write((char*)&environment.description.num_cells.z, sizeof(int));

	ist.write((char*)&environment.description.cell_size, sizeof(double));
	ist.write((char*)&environment.description.cell_size, sizeof(double));
	ist.write((char*)&environment.description.cell_size, sizeof(double));
