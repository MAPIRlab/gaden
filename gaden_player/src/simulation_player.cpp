...
void sim_obj::read_headers(std::stringstream& inbuf, std::string& line) {
	...
	line.erase(0, pos + 1);
	pos = line.find(" ");
	environment.description.num_cells.y = atoi(line.substr(0, pos).c_str());
	environment.description.num_cells.z = atoi(line.substr(pos + 1).c_str());
	...
	pos = line.find(" ");
	environment.description.cell_size = atof(line.substr(0, pos).c_str());
...
void sim_obj::load_binary_file(std::stringstream& decompressed) {
	...
	if (first_reading) {
		double bufferD[5];
		decompressed.read((char*)&environment.description.min_coord.x, sizeof(double));
		decompressed.read((char*)&environment.description.min_coord.y, sizeof(double));
		decompressed.read((char*)&environment.description.min_coord.z, sizeof(double));

		decompressed.read((char*)&environment.description.max_coord.x, sizeof(double));
		decompressed.read((char*)&environment.description.max_coord.y, sizeof(double));
		decompressed.read((char*)&environment.description.max_coord.z, sizeof(double));
		...
		decompressed.read((char*)&environment.description.cell_size, sizeof(double));
...
double sim_obj::get_gas_concentration(float x, float y, float z)
{
	...
	if (xx<0 || xx>environment.description.num_cells.x
		|| yy<0 || yy>environment.description.num_cells.y
		|| zz<0 || zz>environment.description.num_cells.z)
	{
	...
	double   end_x, double   end_y, double end_z)
{
	...
	for (int i = 1; i < steps - 1; i++)
	{
		...
		if (environment.Env[indexFrom3D(x_idx, y_idx, z_idx)] != 0) { return false; }
	}
...
int sim_obj::check_pose_with_environment(double pose_x, double pose_y, double pose_z)
{
	...
	if (pose_x<environment.description.min_coord.x || pose_x>environment.description.max_coord.x || pose_y<environment.description.min_coord.y || pose_y>environment.description.max_coord.y || pose_z<environment.description.min_coord.z || pose_z>environment.description.max_coord.z)
		return 1;
			...
			for (int j = 0;j < environment.description.num_cells.y;j++)
			{
				for (int k = 0;k < environment.description.num_cells.z;k++)
				{
					...
					for (int N = 0;N < (int)round(gas_value / 2);N++)
					{
						...
						p.x = environment.description.min_coord.x + (i + 0.5) * environment.description.cell_size + ((rand() % 100) / 100.0f) * environment.description.cell_size;
						p.y = environment.description.min_coord.y + (j + 0.5) * environment.description.cell_size + ((rand() % 100) / 100.0f) * environment.description.cell_size;
						p.z = environment.description.min_coord.z + (k + 0.5) * environment.description.cell_size + ((rand() % 100) / 100.0f) * environment.description.cell_size;
