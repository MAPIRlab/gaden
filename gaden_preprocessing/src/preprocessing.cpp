	...
	cell_state value_to_write)
{
	...
	for (int i = 0; i < triangles.size(); i++)
	{
		...
		bool xLimit = Utils::eq(std::fmod(Utils::max_val(triangles[i][0].x(), triangles[i][1].x(), triangles[i][2].x()) - env_min_x, cell_size), 0) || Utils::eq(std::fmod(Utils::max_val(triangles[i][0].x(), triangles[i][1].x(), triangles[i][2].x()) - env_min_x, cell_size), cell_size);

		bool yLimit = Utils::eq(std::fmod(Utils::max_val(triangles[i][0].y(), triangles[i][1].y(), triangles[i][2].y()) - env_min_y, cell_size), 0) || Utils::eq(std::fmod(Utils::max_val(triangles[i][0].y(), triangles[i][1].y(), triangles[i][2].y()) - env_min_y, cell_size), cell_size);

		bool zLimit = Utils::eq(std::fmod(Utils::max_val(triangles[i][0].z(), triangles[i][1].z(), triangles[i][2].z()) - env_min_z, cell_size), 0) || Utils::eq(std::fmod(Utils::max_val(triangles[i][0].z(), triangles[i][1].z(), triangles[i][2].z()) - env_min_z, cell_size), cell_size);
			...
			for (int height = 0; height < env[0][0].size(); height++)
			{
				...
				if (env[col][row][height] == cell_state::edge)
				{
					...
					if (compare_cell(col + 1, row, height, cell_state::empty) ||
						...
						(compare_cell(col + 1, row + 1, height, cell_state::empty) && env[col][row + 1][height] == cell_state::edge && env[col + 1][row][height] == cell_state::edge))
					{
